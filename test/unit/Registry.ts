...
describe("Registry:Initialization", () => {
  ...
  it("Should get total farms for an account", async () => {
    expect(await registry.landCount(await accounts[2].getAddress())).to.eq(
      zero
    );
  });
});
...
describe("Registry:Attest Property", () => {
  ...
  it("Should attest title property and emit Attestation event", async () => {
    const { attestor, signer } = await signProperty(
      tokenId,
      ipfsHash,
      size,
      accounts[2]
    ...
    await expect(
      ...
      .to.emit(registry, "Attestation")
      .withArgs(tokenId, ipfsHash, size, signer);
  });
  ...
  it("Should not attest duplicate property tokenId", async () => {
    ...
    await expect(
      registry
        .connect(accounts[2])
        .attestProperty(tokenId, ipfsHash2, size, attestor, v, r, s)
  ...
  it("Should not attest invalid property data signed", async () => {
    ...
    await expect(
      registry
        .connect(accounts[2])
        .attestProperty(tokenId2, ipfsHash2, size, attestor, v, r, s)
...
describe("Registry:Claim Ownership", () => {
  ...
  it("Should attest ownership to property", async () => {
    ...
    expect(
      await registry
        .connect(accounts[2])
        .claimOwnership(tokenId, ipfsHash, size, attestor, v, r, s)
    ).to.be.true;
  });
...
describe("Registry#accountProperty", () => {
  ...
  it("Should panic getting total properties for zero address", async () => {
    await expect(token.balanceOf(addressZero)).to.be.revertedWith(
  ...
  it("Should panic get property with invalid index", async () => {
    await expect(
      registry.accountProperty(await accounts[2].getAddress(), 2)
