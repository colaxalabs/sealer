...
task("sealer:deploy", "Deploy Sealer contracts").setAction(
  async (args, hre) => {
    ...
    const token = await (await hre.ethers.getContractFactory("ERC721"))
      .connect(deployer)
      .deploy();
    console.log("Token deployed at ", token.address);

    // deploy registry contract
    const registry = await (await hre.ethers.getContractFactory("Registry"))
      .connect(deployer)
...
function saveToFrontEnd(token, registry, agreement) {
  const contractDir = "/home/lomolo/Projects/galva-web/src/contracts";
  ...
  const tokenArtifactsDir = require(__dirname +
    "/../artifacts/contracts/token/ERC721.sol/ERC721.json");
  const registryArtifactsDir = require(__dirname +
    "/../artifacts/contracts/registry/Registry.sol/Registry.json");
  const agreementArtifactsDir = require(__dirname +
    "/../artifacts/contracts/agreement/Agreement.sol/PropertyUsage.json");
